    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-cail-cream dark:bg-slate-900 px-4">
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-8 max-w-md w-full text-center">
          <p className="text-sm text-gray-600 dark:text-slate-400 mb-4">Invalid reset link. Please request a new one.</p>
    ...
    <div className="min-h-screen flex items-center justify-center bg-cail-cream dark:bg-slate-900 px-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-8 max-w-md w-full">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="font-display font-semibold text-xl text-cail-dark dark:text-slate-200">Choose New Password</h1>
        </div>
        ...
        {done ? (
          <div className="text-center">
            ...
            <p className="text-sm text-gray-600 dark:text-slate-400 mb-6">Your password has been reset.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="password" className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">New Password</label>
              <input id="password" type="password" required minLength={8} value={password} onChange={(e) => setPassword(e.target.value)}
                className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 focus:border-cail-blue focus:ring-2 focus:ring-cail-blue/20 outline-none transition text-sm" placeholder="Min 8 characters" />
            </div>
            <div>
              ...
              <input id="confirm" type="password" required minLength={8} value={confirm} onChange={(e) => setConfirm(e.target.value)}
                className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 focus:border-cail-blue focus:ring-2 focus:ring-cail-blue/20 outline-none transition text-sm" placeholder="Repeat password" />
            </div>
