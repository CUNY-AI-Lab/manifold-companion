...
Session-based cookie auth with session regeneration on login (prevents fixation). Users register as `status: 'pending'` (with optional display name) and cannot log in until admin sets them to `'approved'`. Three statuses: `pending`, `approved`, `disabled`. Two roles: `user`, `admin`. The `/me` endpoint destroys sessions for non-approved accounts and returns `display_name`, `token_allowance`, and current `token_usage`. Users can update profile fields via `PUT /api/auth/profile`, which accepts an object `{ display_name?, onboarded?, theme_preference? }` (all optional). The `onboarded` field dismisses the welcome banner; `theme_preference` stores the dark/light/system setting.
...
- **Onboarding**: `users.onboarded` (INTEGER DEFAULT 0). Set to 1 when user dismisses the welcome banner on first login via `PUT /api/auth/profile`. Dashboard shows welcome banner when `onboarded === 0` and enhanced empty state (two workflow cards) when no projects exist.
...
- **Dashboard usage bars**: Storage and token usage bars displayed at the bottom of the dashboard in a side-by-side layout. Each bar shows `used / total` with color-coded progress (red at 80%). "Request more storage" and "Request more tokens" mailto links compose pre-filled emails to `ailab@gc.cuny.edu` with account email, current usage, and blank fields for requested amount, reason, and project details.
...
- **Registration**: `RegisterPage.jsx` includes optional display name field. `AuthContext` exposes `updateProfile({ display_name?, onboarded?, theme_preference? })` for profile updates.
...
- **Accessibility hooks**: Three hooks in `client/src/hooks/`:
  - `useFocusTrap(containerRef, isOpen)` — traps Tab cycling within modal containers, auto-focuses first element on open, restores focus to trigger on close
  - `useAnnounce()` — returns `announce(message, priority)` for screen reader live regions (`AnnounceProvider` wraps the app in `App.jsx`)
  - `useHotkeys(shortcuts, options)` — keyboard shortcut registration with global registry; `getRegisteredShortcuts()` exports active shortcuts for `KeyboardShortcuts.jsx` to display
- **Keyboard shortcut registry**: Module-level `Map` in `useHotkeys.js`. Components call `useHotkeys({ 'Combo': { handler, label, section, allowInEditable } }, { when })` to register shortcuts. `KeyboardShortcuts.jsx` reads `getRegisteredShortcuts()` to dynamically display all active shortcuts. Platform-aware: `Cmd` maps to `Meta` on Mac, `Ctrl` elsewhere. Supports two-key combos (e.g., `G then D`) via `pendingKeyRef` with 500ms timeout in `Header.jsx`.
- **ARIA and focus management**: All modals use `useFocusTrap` with `role="dialog"`, `aria-modal="true"`, `aria-labelledby`. Tab interfaces use WAI-ARIA `tablist`/`tab`/`tabpanel` pattern with `aria-selected` and `aria-controls`. Icon-only buttons have `aria-label`. Toolbars have `role="toolbar"`. Skip-to-content link in `App.jsx`. `prefers-reduced-motion` and `focus-visible` styles in `index.css`.
- **Editor color controls**: `ColorPicker` component in `HtmlTextDetail.jsx` provides text color and highlight color via `document.execCommand('foreColor')`/`execCommand('hiliteColor')` with a 16-color palette dropdown. Paste sanitization strips `color`, `background-color`, `background` inline styles, `<font>` elements, and `<style>` blocks from pasted HTML to prevent style pollution.
- **Onboarding**: `users.onboarded` column (INTEGER DEFAULT 0). Welcome banner on Dashboard dismisses via `PUT /api/auth/profile` with `{ onboarded: true }`. Enhanced empty state shows two workflow cards (Image-to-Markdown and PDF-to-HTML) when `projects.length === 0 && shared.length === 0`.
